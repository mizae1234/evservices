...
### 7.11 Branch Operating Hours (เวลาเปิด-ปิดทำการของสาขา)
- **Schema**: ใช้ฟิลด์ `OpenTime` และ `CloseTime` ในตารางหลัก **`CM_MsServiceBranch`** โดยตรง (ยกเลิกการใช้ `CM_BranchSlotConfig` แบบระบบเก่าสำหรับเวลาเปิด-ปิดสาขา)
- **UI Settings**: ผู้ใช้งานที่มีสิทธิ์ (ADMIN / SERVICE_CENTER) สามารถตั้งค่าเวลาเข้างานรายสาขาได้ที่ `/service-center/bookings/settings` (เช่น 08:30 - 17:30 น.)
- **Bay Calendar Grid**: ผังปฏิทินช่องซ่อม (Bay Calendar) จะดึง `OpenTime` ของสาขานั้นมาตั้งเป็นเวลาเริ่มต้นคอลัมน์แรกในปฏิทินโดยอัตโนมัติ

### 7.12 Reschedule & Duration Extension (การเลื่อนเวลาและขยายเวลาซ่อม)
- **Auto-Calculation**: เมื่อผู้ใช้งานเปลี่ยนเวลาเริ่มต้น (`StartTime`) ใหม่ในหน้าต่าง Reschedule ระบบจะคำนวณ `EndTime` ให้ใหม่โดยอัตโนมัติ โดยยึด "ระยะเวลาซ่อมเดิม (Duration)" ไว้เสมอ
- **Role Restriction (สิทธิ์การขยายเวลา)**: ผู้ใช้งานระดับสาขาและผู้ดูแลระบบ (ADMIN, SERVICE_CENTER) สามารถกดปุ่ม Quick Add (`+30m`, `+1h`) หรือพิมพ์แก้ไข `EndTime` เองได้ แต่ **สำหรับผู้ใช้งานระดับ `CS` จะถูกซ่อนปุ่มและบล็อกไม่ให้แก้ไขเวลาจบรอบซ่อมได้ (Disable EndTime Field)**

### 7.13 Lunch Break Calculation (การคำนวณข้ามเวลาพักกลางวัน)
- ระบบมีการตั้งค่าล็อคเวลาพักกลางวันไว้ที่ **12:00 - 13:00 น.** เสมอ
- หากระยะเวลาการซ่อม (Duration) กินเวลาพาดผ่านช่วง 12:00 น. ระบบจะทำการ **หยุดนับเวลา 1 ชั่วโมงโดยอัตโนมัติ** (Skipping Lunch Break) 
- *ตัวอย่าง:* เริ่มซ่อม 10:00 น. ใช้เวลา 3 ชั่วโมง (180 นาที) ระบบจะให้ไปจบที่ 14:00 น. แทนที่จะเป็น 13:00 น.
